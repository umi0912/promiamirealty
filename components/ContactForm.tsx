...
export default function ContactForm() {
  ...
  return (
    <div style={{ background: "var(--surface)", borderRadius: 16, border: "1px solid var(--line)", padding: 24 }}>
      ...
      <button onClick={submit} disabled={busy} style={{ width: "100%", background: "var(--coral)", color: "#fff", border: "none", padding: "14px", borderRadius: 999, fontSize: 15, fontWeight: 500, cursor: "pointer", opacity: busy ? 0.6 : 1, fontFamily: "Outfit" }}>{busy ? t("contact.formSending") : t("contact.formSend")}</button>
    </div>
  );
}
