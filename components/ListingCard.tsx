  ...
  return (
    <Link href={`/listings/${l.id}`} style={{ textDecoration: "none", display: "block" }}>
      <div className="lcard" style={{ position: "relative", height: h, borderRadius: 14, overflow: "hidden", background: "var(--surface-2)" }}>
        ...
        <span style={{ position: "absolute", top: 12, left: 12, background: "rgba(44,90,80,.92)", color: "#fff", fontSize: 11, fontWeight: 500, padding: "4px 10px", borderRadius: 999 }}>{l.status}</span>
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: 14 }}>
          <div style={{ color: "#fff", fontSize: 20, fontWeight: 500, fontFamily: "Space Grotesk, sans-serif" }}>{fmtPrice(l.price)}</div>
